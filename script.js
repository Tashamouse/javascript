...
const map = {
  ...
  render(snakePointsArray, foodPoint, preventPoint) {
    ...
    const preventCell = this.cells[`x${preventPoint.x}_y${preventPoint.y}`];

    preventCell.classList.add("prevent");
    this.usedCells.push(preventCell);
  },
};
...
const game = {
  ...
  reset() {
    ...
    this.prevent.setCoordinates(this.getRandomFreeCoordinates());
    this.render();
  },
  ...
  tickHandler() {
    ...
    if (this.prevent.isOnPoint(this.snake.getNextStepHeadPoint())) {
      return this.finish();
    }
